...
Claude Code는 정교한 다층 프롬프트 아키텍처를 사용합니다. 메인 시스템 프롬프트는 고정 문자열이 아니라, 모듈식 section-builder 함수들을 통해 런타임에 동적으로 조립됩니다. 경계 마커가 전역 캐시 가능한 prefix와 세션별 suffix를 구분해 주며, 이를 통해 API 호출 전반에서 프롬프트 캐싱이 가능합니다.

핵심 정체성 프롬프트 외에도, 이 시스템에는 특수 에이전트 프롬프트, 멀티 워커 coordinator, Tool 호출 자동 승인을 위한 2단계 보안 분류기, 그리고 memory selection, session search, tool summarization을 위한 각종 유틸리티 프롬프트가 포함됩니다.
...
| # | 프롬프트 | 설명 |
|---|--------|-------------|
| 05 | [Coordinator 시스템 프롬프트](prompts/05_coordinator_system_prompt.md) | 4단계 워크플로와 동시성 규칙을 갖춘 멀티 워커 오케스트레이터 |
| 06 | [팀메이트 프롬프트 부록](prompts/06_teammate_prompt_addendum.md) | swarm 및 team mode용 통신 프로토콜 |
...
| # | 프롬프트 | 설명 |
|---|--------|-------------|
| 13 | [Tool별 프롬프트](prompts/13_tool_prompts.md) | Bash, Edit, Agent, fork semantics를 포함한 30개 이상의 Tool 설명 |
...
| 17 | [Auto Mode 비평](prompts/17_auto_mode_critique.md) | 사용자가 작성한 auto-mode classifier 규칙을 검토 |
| 20 | [세션 제목](prompts/20_session_title.md) | Haiku 기반 3~7단어 세션 제목 생성기 |
| 29 | [에이전트 요약](prompts/29_agent_summary.md) | coordinator mode에서 서브 에이전트의 진행 상황을 주기적으로 갱신 |
...
| # | 프롬프트 | 설명 |
|---|--------|-------------|
| 21 | [Compact 서비스](prompts/21_compact_service.md) | analysis/summary 블록을 사용하는 다중 변형 대화 요약 |
...
| # | 프롬프트 | 설명 |
|---|--------|-------------|
| 18 | [Proactive Mode](prompts/18_proactive_mode.md) | tick 기반 pacing과 터미널 포커스 인식을 갖춘 자율 에이전트 |
| 23 | [Chrome 브라우저 자동화](prompts/23_chrome_browser_automation.md) | 브라우저 확장 통합: GIF 녹화, 탭 관리, dialog 처리 |
| 24 | [메모리 지침](prompts/24_memory_instruction.md) | CLAUDE.md 로딩, @include 지시어, frontmatter glob 처리 |
...
| 27 | [Remember 스킬](prompts/27_remember_skill.md) | auto-memory 항목을 `CLAUDE.md` 파일로 승격 |
| 28 | [Update Config 스킬](prompts/28_update_config_skill.md) | `settings.json`, hooks, permission 배열을 관리 |

## 아키텍처

### 프롬프트 조립

메인 시스템 프롬프트는 section builder 파이프라인을 통해 구성됩니다.
...
자동 승인 시스템은 아래 요소로 조립되는 별도의 classifier 프롬프트를 사용합니다.

1. **Base prompt**: classifier 지침 포함
2. **Default rules**: allow, deny, environment 섹션 포함
3. **User overrides**: 각 섹션 전체를 대체
4. **2-stage classification**: Stage 1은 빠르게 실행되고, 불확실하면 Stage 2가 확장 추론을 사용
...
| `USER_TYPE=ant` | 내부 전용 섹션과 모델 override를 활성화 |
| Feature flags | proactive mode, verification agents, fork subagents 등을 제어 |
...
- 멀티 에이전트 오케스트레이션과 조정
- 자율 Tool 사용을 위한 보안 분류
- 컨텍스트 윈도 관리와 대화 압축
- 계층적 override 의미 체계를 갖춘 메모리 시스템
