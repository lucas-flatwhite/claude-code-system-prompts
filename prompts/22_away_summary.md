...
시스템 프롬프트는 선택적 세션 메모리 prefix와 함께 동적으로 조립됩니다.
...
- **Brevity**: 정확히 1~3개의 짧은 문장
- **Task-first**: 사용자가 무엇을 만들거나 디버깅하는지부터 시작
- **Actionable**: 구체적인 다음 단계를 포함
- **No noise**: 상태 보고와 commit 요약은 생략
- **Implementation detail avoidance**: 코드 세부사항이 아니라 상위 수준 작업에 집중

## 입력 처리

- 큰 세션에서 "prompt too long" 오류를 피하기 위해 마지막 30개 메시지(`RECENT_MESSAGE_WINDOW = 30`)만 사용합니다.
- 더 넓은 컨텍스트를 위해 세션 메모리 내용을 포함합니다.
- 응답 속도를 위해 thinking을 비활성화합니다.
- 캐시 오염을 피하기 위해 `skipCacheWrite: true`를 사용합니다.

## 오류 처리

- abort, 빈 transcript, 어떤 오류든 발생 시 `null`을 반환합니다.
