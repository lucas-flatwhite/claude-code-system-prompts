...
## 작업 수행 섹션 (`getSimpleDoingTasksSection`)
...
## Tool 사용 섹션 (`getUsingYourToolsSection`)
...
## 톤 및 스타일 섹션 (`getSimpleToneAndStyleSection`)
...
## 출력 효율성 섹션 (`getOutputEfficiencySection`)
...
### Anthropic 내부 사용자 (USER_TYPE=ant)
...
이 마커 위쪽은 `scope: 'global'`로 캐시할 수 있고, 아래쪽은 세션별 내용입니다.

---

## 세션별 지침 (`getSessionSpecificGuidanceSection`)
...
## 스크래치패드 지침 (`getScratchpadInstructions`)
...
## 함수 결과 정리 (`getFunctionResultClearingSection`)
...
### 숫자 길이 앵커 (Anthropic 내부 전용)
```
Length limits: keep text between tool calls to ≤25 words. Keep final responses to ≤100 words unless the task requires more detail.
```

### 토큰 예산 (Feature-flagged)
```
