...
coordinator mode에서 실행 중인 서브 에이전트의 백그라운드 진행 상황을 주기적으로 생성합니다. 각 워커가 무엇을 하고 있는지 부모 에이전트가 실시간으로 파악할 수 있게 합니다.
...
- **Single sentence**: 최대 한 문장, 현재 시제
- **Action-specific**: 전체 목표가 아니라 현재 동작을 설명
- **No meta-commentary**: 에이전트 자체를 설명하지 말고, 하고 있는 일을 설명
- **Present tense**: 항상 현재 시제 동사 사용 ("Reading", "Running", "Fixing")

## 통합 지점

- coordinator mode에서 주기 타이머로 실행됩니다.
- 결과는 coordinator의 status view에 표시됩니다.
